...
// ==================== ROTAS DE PROPHET/PREVISÕES ====================

// Buscar previsões de um usuário
fastify.get<{
  Params: { userId: string }
}>('/prophet/predictions/:userId', async (request, reply) => {
  try {
    const { userId } = request.params

    fastify.log.info({ userId }, '🔮 Buscando previsões do usuário')

    const { data, error } = await supabase
      .rpc('get_user_predictions_v2', { p_user_id: userId })

    if (error) {
      fastify.log.error({ err: error }, '❌ Erro ao buscar previsões')
      return reply.code(500).send({ error: 'Erro ao buscar previsões', details: error })
    }

    fastify.log.info({ count: data?.length }, '✅ Previsões encontradas')
...
fastify.get<{
  Params: { userId: string }
}>('/prophet/stats/:userId', async (request, reply) => {
  try {
    const { userId } = request.params

    fastify.log.info({ userId }, '📊 Buscando estatísticas do usuário')

    const { data, error } = await supabase
      .rpc('get_user_prediction_stats_v2', { p_user_id: userId })

    if (error) {
      fastify.log.error({ err: error }, '❌ Erro ao buscar estatísticas')
      return reply.code(500).send({ error: 'Erro ao buscar estatísticas', details: error })
    }
    ...
    fastify.log.info({ stats }, '✅ Estatísticas encontradas')
...
}>('/prophet/predictions', async (request, reply) => {
  try {
    const authHeader = request.headers.authorization
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return reply.code(401).send({ error: 'Token não fornecido' })
    ...
    const { trackId, trackData, predictedViralDate, pointsBet, predictionConfidence, predictionType, initialPopularity } = request.body
    ...
    if (pointsBet < 10 || pointsBet > 1000) {
      return reply.code(400).send({ error: 'Pontos devem estar entre 10 e 1000' })
    }

    if (predictionType !== 'increase' && predictionType !== 'decrease') {
      return reply.code(400).send({ error: 'Tipo de previsão inválido' })
    }

    // Verificar pontos do usuário
    const { data: userPoints } = await supabase.rpc('get_user_points', { user_uuid: user.id })

    if (!userPoints || userPoints < pointsBet) {
      return reply.code(400).send({ 
        error: `Pontos insuficientes. Você tem ${userPoints || 0} pontos, mas precisa de ${pointsBet}.` 
      })
    }

    // Verificar/Criar prediction_track
    ...
    if (existingPredictionTrack.data) {
      ...
      if (predictionTrackError) {
        fastify.log.error({ err: predictionTrackError }, 'Erro ao criar prediction track')
        return reply.code(500).send({ error: 'Erro ao salvar música' })
      }
    ...
    if (existingPrediction) {
      return reply.code(400).send({ error: 'Você já fez uma previsão para esta música' })
    }
    ...
    if (predictionError) {
      fastify.log.error({ err: predictionError }, 'Erro ao criar previsão')
      return reply.code(500).send({ error: 'Erro ao criar previsão' })
    }

    // Debitar pontos
    const { data: debitResult } = await supabase.rpc('debit_user_points', {
      user_uuid: user.id,
    ...
    if (!debitResult) {
      await supabase.from('music_predictions_v2').delete().eq('id', prediction.id)
      return reply.code(500).send({ error: 'Erro ao debitar pontos' })
    }

    fastify.log.info({ 
      predictionId: prediction.id,
      userId: user.id,
      trackTitle: trackData.track_title
...
fastify.post('/prophet/process-expired', async (request, reply) => {
  try {
    const authHeader = request.headers.authorization
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return reply.code(401).send({ error: 'Token não fornecido' })
    ...
    fastify.log.info('⏰ Processando previsões expiradas...')

    const { data, error } = await supabase.rpc('process_expired_predictions_v2')

    if (error) {
      fastify.log.error({ err: error }, '❌ Erro ao processar previsões expiradas')
      return reply.code(500).send({ error: 'Erro ao processar previsões', details: error })
    }

    fastify.log.info({ count: data?.length }, '✅ Previsões processadas')
...
const start = async () => {
  try {
    ...
    console.log('📋 Rotas de Prophet/Previsões:')
    console.log('   GET  /prophet/predictions/:userId - Buscar previsões de um usuário')
    console.log('   GET  /prophet/stats/:userId - Buscar estatísticas de um usuário')
    console.log('   POST /prophet/predictions - Criar nova previsão')
    console.log('   POST /prophet/process-expired - Processar previsões expiradas')
